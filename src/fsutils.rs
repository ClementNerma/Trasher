...
pub fn determine_trash_dir_for(item: &Path) -> Result<PathBuf> {
    debug!("Determine trasher directory for item: {}", item.display());
...
/// Determine path to the trash directory for a given item
pub fn determine_trash_dir_for_no_create(item: &Path) -> Result<PathBuf> {
...
pub fn list_trash_dirs() -> Result<Vec<PathBuf>> {
    let canon_root = fs::canonicalize("/").context("Failed to canonicalize the root directory")?;

    let mut trash_dirs = mountpaths()
        .context("Failed to list system mountpoints")?
        .iter()
        .map(|path| determine_trash_dir_for_no_create(path))
        .collect::<Result<Vec<_>, _>>()?;

    if !trash_dirs.contains(&canon_root) {
        trash_dirs.push(canon_root);
    }
